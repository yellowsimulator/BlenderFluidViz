...
def solver(I, V, f, q, Lx, Ly, Nx, Ny, dt, T, b,newman_bc, dirichlet_bc,
           user_action=None, version='scalar'):
    ...
    if version == 'scalar':
        ...
        u = advance_scalar(
            u, u_1, u_2, f, x, y, t, n, Cx2, Cy2, b, newman_bc, dirichlet_bc,
            q, dt2, V, step1=True)
        ...
        try:
            ...
            u = advance_vectorized(
                u, u_1, u_2, f_a, Cx2, Cy2, b, newman_bc, dirichlet_bc, 
                q_a, dt2, V=True, step1=True)
    ...
    for n in It[1:-1]:
        if version == 'scalar':
            ...
            u = advance(u, u_1, u_2, f, x, y, t, n, Cx2, Cy2,
                         b,newman_bc, dirichlet_bc, q, dt2)
        else:
            f_a[:,:] = f(xv, yv, t[n])  # precompute, size as u
            q_a[:,:] = q(xv, yv)
            u = advance(u, u_1, u_2, f_a, Cx2, Cy2,
                         b, newman_bc, dirichlet_bc, q_a, dt2)
...
def advance_scalar(u, u_1, u_2, f, x, y, t, n, Cx2, Cy2, b, newman_bc, dirichlet_bc,
...
def advance_vectorized(u, u_1, u_2, f_a, Cx2, Cy2, b, 
                       newman_bc, dirichlet_bc, q_a, dt2, V=True, step1=False):
    ...
    q_xu_xx =  0.5*(q_a[1:-1,1:-1] + q_a[2:,1:-1])*(u_1[2:,1:-1]-u_1[1:-1,1:-1]) \
                      -0.5*(q_a[1:-1,1:-1] + q_a[:-2,1:-1])*(u_1[1:-1,1:-1]-u_1[:-2,1:-1])
                      
    q_yu_yy =  0.5*(q_a[1:-1,1:-1] + q_a[1:-1,2:])*(u_1[1:-1,2:]-u_1[1:-1,1:-1]) \
                      -0.5*(q_a[1:-1,1:-1] + q_a[1:-1,:-2])*(u_1[1:-1,1:-1]-u_1[1:-1,:-2])
...
def gaussian(initial_surface=3, plot_method=2, version='vectorized', save_plot=True):
    """
    ...
    b = 0.1
    def q(x,y):
        return 1./(1+numpy.exp(-x-y))
    newman_bc=True; dirichlet_bc=False
    dt, cpu = solver(I, None, None, q, Lx, Ly, Nx, Ny, -1, T, b, newman_bc, dirichlet_bc,
...
#mencoder "mf://*.png" -mf fps=10 -o anim.avi -ovc lavc -lavcopts vcodec=msmpeg4v2:vbitrate=500

#I, V, f, q, Lx, Ly, Nx, Ny, dt, T, b,
#user_action=None, version='scalar'
if __name__ == '__main__':
    #test_quadratic()
    gaussian(initial_surface=4, plot_method=3, version='scalar', save_plot=True)
