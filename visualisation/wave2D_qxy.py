...
def solver(I, V, f, q, Lx, Ly, Nx, Ny, dt, T, b,newman_bc, dirichlet_bc,
    ...
    for n in It[1:-1]:
        if version == 'scalar':
            ...
            f_a[:,:] = f(xv, yv, t[n])  # precompute, size as u
            u = advance(u, u_1, u_2, f_a, x, y, xv, yv, Cx2, Cy2,
                         b, newman_bc, dirichlet_bc, q, dt2)
...
def advance_vectorized(u, u_1, u_2, f, x, y, xv, yv, Cx2, Cy2, b, 
                       newman_bc, dirichlet_bc, q, dt2, V=True, step1=False):
    pass
#     
#     if step1:
#         dt = sqrt(dt2)
...
#     if newman_bc:
#         r = 0
#     q_xu_xx =  0.5*(q(x[:],y[:]) + q(x[1:],y[]))*(u_1[1:,:]-u_1[:,:]) \
#                 -0.5*(q(xv,yv) + q(x[:-1],yv))*(u_1[:,:]-u_1[:-1,:])
#     q_yu_yy =  0.5*(q(xv,yv) + q(xv,y[1:]))*(u_1[:,1:]-u_1[:,:]) \
#                 -0.5*(q(xv,yv) + q(xv,y[:-1]))*(u_1[:,:]-u_1[:,:-1])
#     u[:,:]  = D1*u_1[:,:] - D2*u_2[:,:] + Cx2*q_xu_xx + Cy2*q_yu_yy \
#                 + dt2*f_a
#     if step1:
#         u[:,:] += (1-0.5*b*dt)*dt*V(x[:], y[:])
#     if dirichlet_bc:
#         j = 0
#         u[:,j] = 0
#         j = u.shape[1]-1
...
def gaussian(initial_surface=3, plot_method=2, version='vectorized', save_plot=True):
    ...
    if initial_surface == 1:
        ...
        def I(x,y):
            ...
            return B0 + Ba*exp(-Kx - Ky)
    elif initial_surface == 4:
        path = 'cosinus_hat_2Dx'
        def I(x,y):
            B0 = 0. ; Ba = 0.1; b = 1.
            Bs = sqrt(2); Bmx = 0; Bmy = 0
            Kx = cos(pi*((x-Bmx)/(2*Bs))); Ky = cos(pi*((y-Bmy)/(2*Bs)))
            return B0 + Ba*Kx*Ky
...
if __name__ == '__main__':
    #test_quadratic()
    gaussian(initial_surface=3, plot_method=3, version='scalar', save_plot=True)
